...
public class HigherOrderUtils {
    ...
    public interface NamedBiFunction<T,R,K> extends BiFunction<T,R,K>{
        String name();
    }
    ...
    public static <T> T zip(List<T> args, List<? extends BiFunction<T, T, T>> bifunctions){
        if (args.size() != bifunctions.size() + 1) {
            throw new IllegalArgumentException("Unable zip");
        }
        else {
            int i;
            for(i = 0; i < bifunctions.size(); ++i) {
...
// different use case, not with NamedBiFunction objects
        List<String> strings = Arrays.asList("a", "n", "t","s");
// note the syntax of this lambda expression
        BiFunction<String, String, String> concat = (s, t) -> s + t;
        String s = zip(strings, Arrays.asList(concat, concat,concat)); // expected correct value: "ant"
